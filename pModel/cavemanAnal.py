...
def performPlotTwo(age):
    ...
    for i in range(len(stabs)):
        ...
        ax = sns.heatmap(stabilities, linewidth=0.5,vmin=0,vmax=1,cmap='rocket_r').set(title=(str(age)+"% external communication in p model"))
        plt.xlabel ('Agent ID')
        plt.ylabel ('Agent ID')
        plt.show()
...
def getStabs():
    ...
    plt.title("Mean language stability in p Model with standard deviation")
    plt.tight_layout()
    plt.legend()
    plt.show()
