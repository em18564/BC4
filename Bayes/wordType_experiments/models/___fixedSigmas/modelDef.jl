...
@model function model_exp(participant,word,surprisal,tags,PCA,ExpMean,cauchyMean,NUM_TYPES,NUM_PARTICIPANTS)
  ...
  a_ws ~ filldist(Normal(0, 0.05),NUM_TYPES)
  b_ws ~ filldist(Normal(0, 0.05),NUM_TYPES)
  a_w = a_ws[tags.+1]
  b_w = b_ws[tags.+1]

  a_ps ~ filldist(Normal(0, 0.2),NUM_PARTICIPANTS)
  b_ps ~ filldist(Normal(0, 0.05),NUM_PARTICIPANTS)
