...
@model function model(participant,word,surprisal,tags,eEGs)
  ...
  trans = CorrCholeskyFactor(6)
  R_tilde ~ filldist(Turing.Flat(), dimension(trans))
  R_U, logdetJ = transform_and_logjac(trans, R_tilde)
  Turing.@addlogprob! logpdf(LKJCholesky(6, 2), Turing.Cholesky(R_U)) + logdetJ

  σ_σ ~ filldist(Exponential(10), 6)
  Σ_L = LowerTriangular(collect(σ_σ .* R_U')) # collect is necessary for ReverseDiff for some reason
  Σ_σ = PDMat(Cholesky(Σ_L))
