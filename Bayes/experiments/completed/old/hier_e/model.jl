...
@model function model(participant,word,surprisal,tags,eLAN,lAN,n400,ePNP,p600,pNP)
  ...
  a_e = ab_e[1,1]
  b_e = ab_e[2,1]
  μ_eLAN = @. a + a_e + ((b + b_e) * surprisal)
  a_e = ab_e[1,2]
  b_e = ab_e[2,2]
  μ_lAN = @. a + a_e + ((b + b_e) * surprisal)
  a_e = ab_e[1,3]
  b_e = ab_e[2,3]
  μ_n400 = @. a + a_e + ((b + b_e) * surprisal)
  a_e = ab_e[1,4]
  b_e = ab_e[2,4]
  μ_ePNP = @. a + a_e + ((b + b_e) * surprisal)
  a_e = ab_e[1,5]
  b_e = ab_e[2,5]
  μ_p600 = @. a + a_e + ((b + b_e) * surprisal)
  a_e = ab_e[1,6]
  b_e = ab_e[2,6]
  μ_pNP = @. a + a_e + ((b + b_e) * surprisal)

  σ_O = @. a_w + a_p + ((b_w + b_p) * surprisal)
  ...
  for i in eachindex(participant)
    σ ~ truncated(Cauchy((σ_O[i])),0,1000)
