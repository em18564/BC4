...
@model function model(participant,eRP,word,surprisal,tags,component)
    ...
    for i in eachindex(eRP)
        #ζ ~ Normal(0,1)
        μ = (((a[Int(word[i])] * σ_a) + (a_p[Int(participant[i]+1)] * σ_ap) + (a_w[Int(tags[i]+1)] * σ_aw) + (a_e * σ_ae))
        +    ((b[Int(word[i])] * σ_b) + (b_p[Int(participant[i]+1)] * σ_bp) + (b_w[Int(tags[i]+1)] * σ_bw) + (b_e * σ_be)) * surprisal[i])
        eRP[i] ~ Normal(μ,σ)
      end
end
