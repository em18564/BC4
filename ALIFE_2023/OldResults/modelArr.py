...
class LearningAgent(Agent):
    def __init__(self, id, model, signalSize, meaningSize, hiddenSize, utterances = 30,epochs = 15):
    ...
    def step(self):
        p = self.random.uniform(0, 1)
        self.age += 1
        if p < self.age * 0.0005 and death and self.age > 5 and self.stepVal%200<180:
            self.witnesses = np.zeros((len(self.meaningSpace),len(self.signalSpace)))
            self.W1             = np.random.randn(self.signalSize[0], self.hiddenSize)
            self.W2             = np.random.randn(self.hiddenSize, self.meaningSize[0])
            self.age = 0
            #print("AGENT ",self.id," DIED")
        elif p < 0.25 and self.age > 5:
            self.invent()
        if self.age == 5 and death:
            self.generateObvert()
...
def main(argv):
    global probability
    if len(argv)!=0:
        probability = 0.01 + float(argv[0])/1000
